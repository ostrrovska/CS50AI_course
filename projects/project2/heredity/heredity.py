...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person, info in people.items():
        ...
        if info["mother"] is None and info["father"] is None:
            ...
            father = info["father"]
            pass_probs = {
                mother: (1 - PROBS["mutation"]) if mother in two_genes else
                0.5 if mother in one_gene else
                PROBS["mutation"],
                father: (1 - PROBS["mutation"]) if father in two_genes else
                0.5 if father in one_gene else
                PROBS["mutation"]
            }
            if genes == 2:
                prob_gene = pass_probs[mother] * pass_probs[father]
            elif genes == 1:
                prob_gene = pass_probs[mother] * (1 - pass_probs[father]) + \
                            pass_probs[father] * (1 - pass_probs[mother])
            else:
                prob_gene = (1 - pass_probs[mother]) * (1 - pass_probs[father])

        # Probability of trait
        prob_trait = PROBS["trait"][genes][person in have_trait]

        # Combine probabilities
        total_prob *= prob_gene * prob_trait
